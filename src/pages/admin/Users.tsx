...
export default function UsersPage() {
  ...
  // Safely get users with proper error handling
  const { data: users = [], isLoading, error: queryError } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        // Apply filters
        if (filters && filters.subscription && filters.subscription !== 'all') {
  ...
  // Set loaded state after initial query
  useEffect(() => {
    if (!isLoading) {
      setIsLoaded(true);
    }
  ...
  // Safe edit user function
  const handleEditUser = async (updatedProfile) => {
    if (!selectedUser || !updatedProfile) return;
  ...
  // Safe rendering for error state
  if (queryError) {
  ...
  // Safe calculation of active filters
  const activeFiltersCount = 
    (filters ? Object.values(filters).filter(value => value !== 'all').length : 0) + 
    (searchQuery ? 1 : 0);
