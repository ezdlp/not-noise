...
const NewCampaignButton = ({ isPro, onOpenModal }: { isPro: boolean; onOpenModal: () => void }) => {
  return (
    <Button onClick={onOpenModal}>
      <PlusCircleIcon className="mr-2 h-4 w-4" />
      New Campaign {isPro && <span className="ml-1">(10% off)</span>}
    </Button>
  );
};
...
export function PromotionsDashboard() {
  ...
  const { data: promotions, isLoading, refetch: refetchPromotions } = useQuery({
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="flex items-center space-x-2 text-sm text-muted-foreground">
              ...
              <span>•</span>
              <span>{formatDistance(new Date(campaign.created_at), new Date(), { addSuffix: true })}</span>
            </div>
          </div>
...
function EmptyPromotionsState({ isPro, onOpenModal }: { isPro: boolean; onOpenModal: () => void }) {
  return (
