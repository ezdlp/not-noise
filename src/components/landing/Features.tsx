...
const CustomTooltip = ({
  ...
  if (active && payload && payload.length) {
    return <div className="bg-white/50 p-3 md:p-4 border border-neutral-100/60 rounded-lg shadow-[0_2px_4px_rgba(0,0,0,0.02)]">
        <p className="text-xs md:text-sm font-medium text-neutral-600 mb-2">{label}</p>
        {payload.map((pld: any, index: number) => <div key={index} className="flex items-center gap-1.5 md:gap-2 mb-1">
...
const SmartLinkShowcase = () => {
  ...
  return <div className="mt-8 md:mt-12 relative">
      {/* Mobile Layout */}
      <div className="md:hidden relative">
        ...
        <div className="overflow-hidden px-4">
          ...
          loop: true
            ...
            <CarouselContent>
              {smartLinks.map((link, index) => <CarouselItem key={index}>
                  <div className="relative py-6">
                    <img src={link.image} alt={`Smart Link Example ${index + 1}`} className="w-full shadow-[0_2px_4px_rgba(0,0,0,0.02)] rounded-xl mx-auto max-w-[280px]" />
                  </div>
                </CarouselItem>)}
            </CarouselContent>
          </Carousel>
          ...
          <div className="flex justify-center gap-2 mt-4">
            {smartLinks.map((_, index) => <button key={index} onClick={() => api?.scrollTo(index)} className={cn("w-2 h-2 rounded-full transition-colors", index === currentIndex ? 'bg-primary' : 'bg-neutral-200')} />)}
          </div>
        </div>
      </div>
      ...
      <div className="hidden md:block">
        <div className="flex justify-center relative py-6 md:py-8">
          {smartLinks.map((link, index) => <div key={index} className="flex-none w-[220px] group relative" style={{
          ...
          zIndex: index
              ...
              <div className="relative transition-all duration-300 group-hover:rotate-0 group-hover:-translate-y-4 group-hover:z-50">
                <img src={link.image} alt={`Smart Link Example ${index + 1}`} className="w-full shadow-[0_2px_4px_rgba(0,0,0,0.02)] rounded-xl" />
              </div>
            </div>)}
        </div>
      </div>
      ...
      <div className="max-w-xl mx-auto">
        <div className="bg-white/50 rounded-xl shadow-[0_2px_4px_rgba(0,0,0,0.02)] border border-neutral-100/60 overflow-hidden">
          <div className="flex justify-center gap-3 md:gap-4 pt-4 md:pt-6 pb-3 md:pb-4">
            <Toggle pressed={activeFormat === 'post'} onPressedChange={() => setActiveFormat('post')} className={`px-4 md:px-6 py-1.5 md:py-2 text-xs md:text-sm font-medium transition-all ${activeFormat === 'post' ? 'bg-primary text-white shadow-[0_2px_4px_rgba(0,0,0,0.02)]' : 'text-neutral-600 hover:bg-neutral-50'}`}>
              Post
            </Toggle>
            <Toggle pressed={activeFormat === 'story'} onPressedChange={() => setActiveFormat('story')} className={`px-4 md:px-6 py-1.5 md:py-2 text-xs md:text-sm font-medium transition-all ${activeFormat === 'story' ? 'bg-primary text-white shadow-[0_2px_4px_rgba(0,0,0,0.02)]' : 'text-neutral-600 hover:bg-neutral-50'}`}>
              Story
            </Toggle>
          </div>
          ...
          <div className="h-[360px] md:h-[480px] flex items-center justify-center bg-neutral-50 p-4 md:p-6">
            <div className={`h-full ${activeFormat === 'post' ? 'w-full' : 'w-[200px] md:w-[270px]'}`}>
              <img src={activeFormat === 'post' ? 'https://owtufhdsuuyrgmxytclj.supabase.co/storage/v1/object/public/media-library/taylor-post.jpg' : 'https://owtufhdsuuyrgmxytclj.supabase.co/storage/v1/object/public/media-library/taylor-story.jpg'} alt="Social media preview" className="w-full h-full object-contain rounded-lg shadow-[0_2px_4px_rgba(0,0,0,0.02)]" />
            </div>
          </div>
        </div>
      </div>
              ...
              <Card className="border-none bg-card/50 shadow-none">
                ...
                <CardContent className="px-4 pb-4 md:px-6 md:pb-6">
                  <div className="space-y-2">
                    ...
                    name: 'Others',
                    value: 6
                  }].map(platform => <div key={platform.name} className="flex items-center justify-between">
              ...
              <Card className="border-none bg-card/50 shadow-none">
                ...
                <CardContent className="px-4 pb-4 md:px-6 md:pb-6">
                  <div className="space-y-2">
                    ...
                    country: 'Others',
                    value: 4
                  }].map(geo => <div key={geo.country} className="flex items-center justify-between">
              ...
              <Card className="border-none bg-card/50 shadow-none">
                ...
                <CardContent className="px-4 pb-4 md:px-6 md:pb-6">
                  ...
                  <div className="flex justify-center gap-3 md:gap-4 mt-3 md:mt-4">
                    {deviceData.map((device, index) => <div key={device.name} className="flex items-center gap-1.5 md:gap-2">
                        <div className="w-2.5 h-2.5 md:w-3 md:h-3 rounded-full" style={{
            ...
            <div className="h-[250px] md:h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={mockData}>
                  ...
                  <XAxis dataKey="date" className="text-xs md:text-sm text-neutral-600" tick={{
                  fontSize: 12
                }} tickMargin={8} />
                  <YAxis className="text-xs md:text-sm text-neutral-600" tick={{
                  fontSize: 12
                }} tickMargin={8} />
                  <Tooltip content={<CustomTooltip />} />
                  <Line type="monotone" dataKey="views" stroke="#6851FB" strokeWidth={2} dot={false} activeDot={{
                  r: 4,
                  fill: "#6851FB"
                }} />
                  <Line type="monotone" dataKey="clicks" stroke="#37D299" strokeWidth={2} dot={false} activeDot={{
                  r: 4,
                  fill: "#37D299"
                }} />
                </LineChart>
