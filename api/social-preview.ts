  ...
  try {
    ...
    const html = `
      ...
      <head>
        ...
        <!-- Redirect to the actual page if JavaScript is enabled -->
        <script>
          window.location.href = '/link/${slug}';
        </script>
        
        <!-- Styles for no-JS fallback -->
        <style>
          body {
      ...
      <body>
        <div class="container">
          <img class="artwork" src="${artworkUrl}" alt="${smartLink.title}">
          <h1>${smartLink.title}</h1>
          <p class="artist">by ${smartLink.artist_name}</p>
