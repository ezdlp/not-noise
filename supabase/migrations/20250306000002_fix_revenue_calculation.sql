-- Fix revenue calculation in analytics functions to use accurate stripe data
-- Created: 2025-03-06

-- Update the get_improved_analytics_stats function to use correct revenue data from stripe tables
...
BEGIN
  ...
  FROM (
    ...
    -- Revenue (from stripe charges table)
    revenue AS (
      SELECT 
        date_trunc('day', to_timestamp(created))::date AS day,
        SUM(amount / 100.0) as total -- Convert amount from cents to euros
      FROM stripe_charges
      WHERE to_timestamp(created) BETWEEN p_start_date AND v_end_date
  ...
  FROM (
    ...
    revenue AS (
      SELECT 
        ...
        SUM(amount / 100.0) as total -- Convert amount from cents to euros
      FROM stripe_charges
      WHERE to_timestamp(created) BETWEEN v_prev_start_date AND v_prev_end_date
        AND status = 'succeeded'
      GROUP BY 1
    ),
    -- Smart links created
